...
VsOutput mainVS(float4 pos: POSITION, float2 normalizedUv: TEXCOORD0, float lerpV: TEXCOORD1, float seed: TEXCOORD2, float rot : TEXCOORD3, float size: TEXCOORD4)
{
    ...
    float3 up  = cWorldUp;//normalize(cross(side, eyeVector));
   
    float2 centeredUv = normalizedUv - float2(0.5, 1);
	centeredUv*=size*float2(0.08,2); //nastavovanie velkosti
   
    output.pos = mul(float4(pos + (centeredUv.x * side - centeredUv.y * up), 1), gViewProj);
